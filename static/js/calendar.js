 ...
 */
define(['css/calendar.min.css'],function(require) {
...
(function ($, window, document, undefined) {
        ...
        module = {
          ...
          create: {
            calendar: function () {
              ...
              var columns = isDay ? 7 : isHour ? 4 : 6;
              var columnsString = columns === 7 ? 'seven' : columns === 4 ? 'four' : 'six';
              var rows = isDay || isHour ? 6 : 10;
              ...
              for (p = 0; p < pages; p++) {
                ...
                var dayChange = isHour ? 1 : 0;
				var hourChange = isMinute ? 1 : 0;
				var minuteChange = isSecond ? 1 : 0;
                var prevNextDay = isHour || isMinute || isSecond ? day : 1;
                var prevDate = new Date(year - yearChange, month - monthChange, prevNextDay - dayChange, hour - hourChange, minute - minuteChange);
                var nextDate = new Date(year + yearChange, month + monthChange, prevNextDay + dayChange, hour + hourChange, minute - minuteChange);
                ...
                if (!isTimeOnly) {
                  ...
                  var newMode = isMonth ? (settings.disableYear ? 'day' : 'year') :
                    isDay ? (settings.disableMonth ? 'year' : 'month') : isSecond ?  'minute' : isMinute ?  'hour' : 'day';
                  ...
                  for (c = 0; c < columns; c++, i++) {
                    var cellDate = isYear ? new Date(i, month, 1, hour, minute) :
                      isMonth ? new Date(year, i, 1, hour, minute) : isDay ? new Date(year, month, i, hour, minute) :
                        isHour ? new Date(year, month, day, i) : isMinute ? new Date(year, month, day, hour, i) : new Date(year, month, day, hour, minute, i);
                    var cellText = isYear ? i :
                      isMonth ? settings.text.monthsShort[i] : isDay ? cellDate.getDate() : isSecond ? formatter.Secondtime(cellDate, settings, true) : formatter.time(cellDate, settings, true);
            ...
            keydown: function (event) {
              ...
              if (module.popup('is visible')) {
                if (event.keyCode === 37 || event.keyCode === 38 || event.keyCode === 39 || event.keyCode === 40) {
                  //arrow keys
                  var mode = module.get.mode();
                  var bigIncrement = mode === 'day' ? 7 : mode === 'hour' ? 4 : 6;
          ...
          get: {
            ...
            validModes: function () {
              ...
              if (settings.type.indexOf('time') >= 0) {
				...
				if (!settings.disableSecond) {
                  validModes.push('second');
                }
          ...
          selectDate: function (date, forceSet) {
            var mode = module.get.mode();
            var complete = forceSet || mode === 'second' ||
			  (settings.disableSecond && mode === 'minute' ) ||
            ...
            if (complete) {
              ...
              var newMode = mode === 'year' ? (!settings.disableMonth ? 'month' : 'day') : mode === 'month' ? 'day' : mode === 'day' ? 'hour' : mode === 'hour' ? 'minute' : 'second';
          ...
          helper: {
            ...
            dateDiff: function (date1, date2, mode) {
			  ...
			  var isMinuteOrSecond = isSecond || mode === 'minute'
              ...
              date1 = new Date(
                ...
                !isMinuteOrSecond ? 0 : date1.getMinutes(),
				!isSecond ? 0 : date1.getSeconds() );
              ...
              date2 = new Date(
                ...
                !isMinuteOrSecond ? 0 : date2.getMinutes(),
				!isSecond ? 0 : date2.getSeconds() );
  ...
  $.fn.calendar.settings = {
    ...
    disableSecond: false, // disable minute selection mode
    ...
    formatter: {
          ...
          mode === 'month' ? settings.formatter.monthHeader(date, settings) :
            mode === 'day' ? settings.formatter.dayHeader(date, settings) :
              mode === 'hour' ? settings.formatter.hourHeader(date, settings) :
                mode === 'minute' ? settings.formatter.minuteHeader(date, settings) :
      ...
      minuteHeader: function (date, settings) {
        return settings.formatter.date(date, settings) + " " + date.getHours() + ":";
      },
	  secondHeader: function (date, settings) {
        return settings.formatter.date(date, settings) + " " + date.getHours() + ":" + date.getMinutes() + ":";
      },
      ...
      datetime: function (date, settings) {
        ...
        var time = settings.type.indexOf('time') < 0 ? '' : settings.formatter.outtime(date, settings, false);
	  ...
	  Secondtime: function (date, settings, forCalendar) {
        if (!date) {
          return '';
        };
		var hour = date.getHours();
        ...
        return hour + ':' + (minute < 10 ? '0' : '') + minute + ':' + (second < 10 ? '0' : '') + second + ampm;
      },
	  
	  outtime: function (date, settings, forCalendar) {
        if (!date) {
          return '';
        }
        var hour = date.getHours();
    ...
    parser: {
      date: function (text, settings) {
        ...
        var i, j, k;
        var second = -1, minute = -1, hour = -1, day = -1, month = -1, year = -1;
        var isAm = undefined;
          ...
          for (i = 0; i < numbers.length; i++) {
            ...
            if (number.indexOf(':') >= 0) {
              if (hour < 0 || minute < 0 || second < 0) {
                ...
                for (k = 0; k < Math.min(3, parts.length); k++) {
                  j = parseInt(parts[k]);
                  if (isNaN(j)) {
                    j = 0;
                  }
        ...
        if (date.getMonth() !== month - 1 || date.getFullYear() !== year) {
          ...
          date = new Date(year, month, 0, hour, minute, second);
        }
